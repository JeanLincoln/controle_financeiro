...
export const handleOutcomeSearch = ({
  ...
  if (searchFilters.dateFilter) {
    filteredTransactions = filteredTransactions.filter((transaction) => {
      return format(new Date(transaction.date), 'yyyy-MM-dd') === searchFilters.dateFilter
    })
  }
  ...
  if (searchFilters.valueFilter) {
    filteredTransactions = filteredTransactions.filter((transaction) => {
      return transaction.value.toString().includes(searchFilters.valueFilter.toString())
    })
  }
